...
func scanForTarget(delta):
	...
	# If no per-min thresholds are > 0, keep vanilla behaviour
	var hasAny = false
	...
	var meets = _permin_target_meets_threshold(target, enabled)
	if meets == false:
		target = null
	# if meets == null -> cannot assess -> keep current vanilla target
...
# Geologist's estimated price if available; else market price
...
# true/false if assessed; null if cannot assess (do not block)
func _permin_target_meets_threshold(body, minerals: Array):
	...
	for m in minerals:
		...
		if unit_price <= 0.0:
			# No estimate -> skip; do not block overall selection
			continue

		could_assess_any = true

		# Worth and thresholds are in internal units
		var worth = amount * unit_price

		# print("DRONE worth %s for %s vs min %s" % [worth, m, getMinValueFor(m)])
